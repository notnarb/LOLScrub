...
var mapFunction = function(){
    ...
    if(this.isSoloKill==true){
        emit(this.VictimChampId,{'kills':0,'deaths':1})
        emit(this.KillerChampId,{'kills':1,'deaths':0})
    }
}
...
mongodb.connect ('mongodb://mongo:27017/urfday',function(err,db){
    ...
    function setNextLoop(){
        setTimeout(function(){
            ...
            pointerCollection.find({Process:'OverallKillOdds'}).limit(1).toArray(function(err, docs) {
                if  (err){return console.dir(err);}
                ...
                if(docs.length){
                    ...
                    db.collection('OverallKillOdds').drop();
                    ...
                    db.collection('totalKillCollection').find().sort({_id:-1}).limit(1).toArray(function(err, lastItem) {
                        ...
                        db.collection('totalKillCollection').mapReduce(mapFunction,reduceFunction,{out:{reduce:'OverallKillOdds'},finalize:finalize,query:query,verbose:true},function(err,collection,stats){//finalize:finalize
                            if(err){return console.dir(err);}
                            console.log("Map-Reduce completed")
                            console.log(stats)
                            pointerCollection.findOneAndReplace({Process:'OverallKillOdds'},{Process:'OverallKillOdds', Location:LastItemAdded},{upsert:true},function(err,db){
                                if  (err){return console.dir(err);}
