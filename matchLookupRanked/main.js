...
function initDb() {
	var db;
	mongodb.MongoClient.connectAsync('mongodb://mongo:27017/ranked')
...
function updateMatchListMap (collection) {
	return new Promise (function (resolve, reject) {
		...
		results.filter(function (matchListResult) {
			...
			return !matchListMap[matchListResult._id];
			...
			if (matchListMap[id]) {
				console.log('Match list', id, 'has already been checked, skipping.');
				return;
			}
...
function addSummonerIds(db, matchInfo){
    db.collection('SummonersToLookup')
    matchInfo.participantIdentities.forEach(function (participantIdentity){
           var curSummonerId = participantIdentity.player.summonerId;
           db.collection('SummonersToLookup').find({SummonerId:curSummonerId}).limit(1).toArray(function(err,found){
                if(found[0] == null){
                    db.collection('SummonersToLookup').insert({SummonerId:curSummonerId},function(err,found){
                        console.log("Added Summoner " + curSummonerId + " to list")
                    });

                }
                console.log("Summoner" + curSummonerId + " already in lookup list")
...
function lookupLoop (db) {
	...
	getMatch(matchId).then(function (results) {
		console.log('storing', matchId);123
		addSummonerIds(db,results)
